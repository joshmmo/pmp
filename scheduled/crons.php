<?php			
	...
	*/
	...
	/*
		Expiration Warning Email 30 day
	*/
	// jameel: duplicated pmpro_cron_expiration_warnings and changed to check for 30 day and 60 day notices. Also changed the meta data that determines whether or not this message has gone out already.
	add_action("pmpro_cron_expiration_warnings_30", "pmpro_cron_expiration_warnings_30");
	function pmpro_cron_expiration_warnings_30()
	{	
		global $wpdb;
		...
		$pmpro_email_days_before_expiration = 30;
				
		//look for memberships that are going to expire within one week (but we haven't emailed them within a week)
		$sqlQuery = "SELECT mu.user_id, mu.membership_id, mu.startdate, mu.enddate FROM $wpdb->pmpro_memberships_users mu LEFT JOIN $wpdb->usermeta um ON um.user_id = mu.user_id AND um.meta_key = 'pmpro_expiration_notice_30' WHERE mu.status = 'active' AND mu.enddate IS NOT NULL AND mu.enddate <> '' AND mu.enddate <> '0000-00-00 00:00:00' AND DATE_SUB(mu.enddate, INTERVAL " . $pmpro_email_days_before_expiration . " Day) <= '" . $today . "' AND (um.meta_value IS NULL OR DATE_ADD(um.meta_value, INTERVAL " . $pmpro_email_days_before_expiration . " Day) <= '" . $today . "') ORDER BY mu.enddate";
		...
		{				
			...
			if($send_email)
			{
				...
				printf(__("Membership expiring email (30 day notice) sent to %s. ", "pmpro"), $euser->user_email);
			}
				
			//update user meta so we don't email them again
			update_user_meta($euser->ID, "pmpro_expiration_notice_30", $today);
		}
	}
	/*
		Expiration Warning Emails
	*/
	add_action("pmpro_cron_expiration_warnings_60", "pmpro_cron_expiration_warnings_60");
	function pmpro_cron_expiration_warnings_60()
	{	
		global $wpdb;
		...
		$pmpro_email_days_before_expiration = 60;
				
		//look for memberships that are going to expire within one week (but we haven't emailed them within a week)
		$sqlQuery = "SELECT mu.user_id, mu.membership_id, mu.startdate, mu.enddate FROM $wpdb->pmpro_memberships_users mu LEFT JOIN $wpdb->usermeta um ON um.user_id = mu.user_id AND um.meta_key = 'pmpro_expiration_notice_60' WHERE mu.status = 'active' AND mu.enddate IS NOT NULL AND mu.enddate <> '' AND mu.enddate <> '0000-00-00 00:00:00' AND DATE_SUB(mu.enddate, INTERVAL " . $pmpro_email_days_before_expiration . " Day) <= '" . $today . "' AND (um.meta_value IS NULL OR DATE_ADD(um.meta_value, INTERVAL " . $pmpro_email_days_before_expiration . " Day) <= '" . $today . "') ORDER BY mu.enddate";
		...
		{				
			...
			if($send_email)
			{
				...
				printf(__("Membership expiring email (60 day notice) sent to %s. ", "pmpro"), $euser->user_email);
			}
				
			//update user meta so we don't email them again
			update_user_meta($euser->ID, "pmpro_expiration_notice_60", $today);
		}
	}
